...
export function RegisterPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 8) next.password = register.errors.password
    ...
    setBusy(true)
    const result = await backend.register(email.trim(), password, name.trim(), level)
    setBusy(false)

    if (result.error) return setErrors({ submit: result.error })
    if (result.needsEmailConfirmation) return setConfirmEmail(true)
      ...
      <main className="relative flex min-h-screen items-center justify-center px-5 pb-16 pt-28 md:px-8">
        ...
        >
          <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-gold/50 bg-gold/10">
            <MailCheck className="h-8 w-8 text-gold-light" />
          </span>
          <h1 className="mt-6 font-display text-4xl font-semibold text-parchment">{register.confirmEmail.title}</h1>
          <p className="mt-4 leading-relaxed text-slateink">{register.confirmEmail.text}</p>
          <Button to="/login" variant="outline" className="mt-8">
            {register.confirmEmail.toLogin}
          </Button>
        </motion.div>
      </main>
    )
    ...
    <main className="relative flex min-h-screen items-center justify-center px-5 pb-16 pt-28 md:px-8">
      ...
      >
        ...
        <form onSubmit={onSubmit} className="kontor-card mt-8 space-y-5 p-6 md:p-8" noValidate>
          ...
          <div>
            <label htmlFor="reg-password" className="mb-1.5 block text-sm text-parchment-dim">
            ...
            <input
              ...
              placeholder={register.fields.passwordPlaceholder}
              className="input-field"
